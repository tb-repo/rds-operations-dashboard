...
Traceability: REQ-6.1, REQ-6.2, REQ-6.3, REQ-6.4, REQ-6.5 â†’ DESIGN-001 â†’ TASK-5
        ...
        for instance in instances:
            try:
                ...
                if violations:
                    ...
                    logger.warn(f"Found {len(violations)} violations for {instance['instance_id']}")
                else:
                    compliant_count += 1
...
def get_rds_inventory() -> List[Dict[str, Any]]:
    ...
    table_name = config.dynamodb.inventory_table
...
def create_compliance_alerts(violations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    config = Config.load()
    dynamodb = AWSClients.get_dynamodb_client()
    table_name = config.dynamodb.health_alerts_table
