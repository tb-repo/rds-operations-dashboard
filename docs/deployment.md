# Deployment Guide - Centralized Deployment Model
...
This guide covers deploying the RDS Operations Dashboard using the **centralized deployment model**. In this model:

- **Single dashboard instance** monitors RDS instances across all AWS accounts
- **No environment-based deployments** (no separate dev/staging/prod deployments)
- **RDS instances are classified** by their `Environment` tag (Production, Development, Test, etc.)
- **Simplified resource naming** without environment suffixes
...
```bash
# Your AWS account ID (management account where dashboard is deployed)
...
**Note:** The `ENVIRONMENT` variable has been removed. The dashboard now uses a centralized deployment model where RDS instances are classified by their tags, not by deployment environment.
...
# Deploy data stack (no environment parameter needed)
cdk deploy RDSDashboard-Data
```

This creates:
- DynamoDB tables: `rds-inventory`, `metrics-cache`, `health-alerts`, `audit-log`, `cost-snapshots`, `rds-approvals`
- S3 bucket: `rds-dashboard-data-{account-id}` (no environment suffix)
...
Outputs:
RDSDashboard-Data.RdsInventoryTableName = rds-inventory
RDSDashboard-Data.MetricsCacheTableName = metrics-cache
RDSDashboard-Data.HealthAlertsTableName = health-alerts
RDSDashboard-Data.AuditLogTableName = audit-log
RDSDashboard-Data.CostSnapshotsTableName = cost-snapshots
RDSDashboard-Data.ApprovalsTableName = rds-approvals
RDSDashboard-Data.DataBucketName = rds-dashboard-data-123456789012
```
...
```bash
cd ../scripts
python setup-s3-structure.py --account-id 123456789012
```
...
```powershell
cd ..\scripts
.\setup-s3-structure.ps1 -AccountId "123456789012"
```
...
```bash
# Deploy all stacks at once using the deployment script
cd ../scripts
./deploy-all.ps1
...
# Deploy compute stack (Lambda functions)
cdk deploy RDSDashboard-Compute

# Deploy API stack (API Gateway)
cdk deploy RDSDashboard-API

# Deploy auth stack (Cognito)
cdk deploy RDSDashboard-Auth

# Deploy BFF stack (Backend for Frontend)
cdk deploy RDSDashboard-BFF

# Deploy orchestration stack (Step Functions)
cdk deploy RDSDashboard-Orchestration

# Deploy monitoring stack (CloudWatch dashboards)
...
Outputs:
RDSDashboard-IAM.LambdaExecutionRoleArn = arn:aws:iam::123456789012:role/RDSDashboardLambdaRole
RDSDashboard-IAM.ExternalId = rds-dashboard-unique-id-12345
```
...
aws s3 ls s3://rds-dashboard-data-123456789012/ --recursive
```
...
aws iam get-role --role-name RDSDashboardLambdaRole

# List attached policies
aws iam list-attached-role-policies --role-name RDSDashboardLambdaRole
```
...
- One deployment monitors all RDS instances across all AWS accounts
- No separate dev/staging/prod deployments of the dashboard itself

**RDS Instance Classification:**
- RDS instances are classified by their `Environment` tag
- Supported classifications: Production, Development, Test, Staging, UAT, etc.
- Authorization is based on RDS instance environment, not deployment environment

**Simplified Resource Naming:**
- Stack names: `RDSDashboard-Data`, `RDSDashboard-Compute`, etc. (no `-prod` suffix)
- Table names: `rds-inventory`, `metrics-cache`, etc. (no `-prod` suffix)
- Lambda names: `rds-discovery`, `rds-operations`, etc. (no `-prod` suffix)

### Migration from Environment-Based Deployment

If you're migrating from an environment-based deployment:

1. **Data Migration:** Export data from old tables (`rds-inventory-prod`) to new tables (`rds-inventory`)
2. **Update Cross-Account Roles:** Update trust relationships to reference new role names
3. **Update Frontend Configuration:** Update API endpoints to point to new API Gateway
4. **Decommission Old Stacks:** After validation, destroy old environment-specific stacks
...
- [ ] RDS instances tagged with `Environment` tag for proper classification
- [ ] User permissions configured based on RDS environment classification
